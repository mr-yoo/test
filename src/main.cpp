...
constexpr unsigned char ALPHA_OPAQUE = 0xFF;

// Helper function for PNG cleanup
void cleanup_png_read(png_structp png, png_infop info, FILE *fp) {
  if (png && info) {
...
void cleanup_png_write(png_structp png, png_infop info, FILE *fp) {
  if (png && info) {
...
bool read_png(const char *filename, std::vector<unsigned char> &image,
  ...
  if (!png) {
    cleanup_png_read(nullptr, nullptr, fp);
    return false;
  }

  png_infop info = png_create_info_struct(png);
  if (!info) {
    cleanup_png_read(png, nullptr, fp);
    return false;
  }

  if (setjmp(png_jmpbuf(png))) {
    cleanup_png_read(png, info, fp);
    return false;
  }
  ...
  png_set_filler(png, ALPHA_OPAQUE, PNG_FILLER_AFTER);
...
bool write_png(const char *filename, const std::vector<unsigned char> &image,
  ...
  if (!png) {
    cleanup_png_write(nullptr, nullptr, fp);
    return false;
  }

  png_infop info = png_create_info_struct(png);
  if (!info) {
    cleanup_png_write(png, nullptr, fp);
    return false;
  }

  if (setjmp(png_jmpbuf(png))) {
    cleanup_png_write(png, info, fp);
    return false;
  }
  ...
  png_size_t rowbytes = width * RGBA_CHANNELS;
  std::vector<png_bytep> rows(height);
  for (size_t y = 0; y < height; ++y) {
...
horizontal_lowpass(const std::vector<unsigned char> &src, unsigned width,
                   unsigned height) {
  std::vector<unsigned char> dst(src.size());

  for (unsigned y = 0; y < height; ++y) {
    for (unsigned x = 0; x < width; ++x) {
      for (int c = 0; c < RGBA_CHANNELS; ++c) {
        auto sample = [&](int xx) {
          int nx = std::clamp(xx, 0, (int)width - 1);
          return src[(y * width + nx) * RGBA_CHANNELS + c];
        };
        int sum = sample(x - 1) + 2 * sample(x) + sample(x + 1);
        dst[(y * width + x) * RGBA_CHANNELS + c] =
